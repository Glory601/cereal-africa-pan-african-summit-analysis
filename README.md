# Cereal+ Africa Pan African Summit — Registration Analytics

## 📊 Project Overview

This project analyzes registration data for the Cereal+ Africa Pan African Summit to evaluate campaign performance, understand participant demographics, identify effective registration channels, and monitor progress toward the registration target.

The analysis combines registration tracking with Power BI visualization to turn raw registration data into actionable business insights.

---

## 🎯 Business Problem

The summit team needed a clear and reliable way to monitor registration performance throughout the campaign.

The key questions were:

- How many participants had registered?
- How close were registrations to the target?
- Which channels generated the most registrations?
- Who were the major participant groups?
- Where were participants registering from?
- How did registration performance change over time?
- What actions could help improve registration performance?

---

## 🎯 Project Objectives

1. Track registration performance against the target.
2. Analyze participant categories and geographic distribution.
3. Identify the strongest registration sources.
4. Monitor registration trends and week-over-week growth.
5. Present findings through an interactive Power BI dashboard.
6. Generate practical recommendations for improving campaign performance.

---

## 🛠️ Tools & Skills

### Tools
- Microsoft Power BI
- Google Sheets
- Microsoft Excel
- GitHub

### Skills
- Data Cleaning
- Data Analysis
- Data Visualization
- Trend Analysis
- Campaign Performance Analysis
- Business Reporting
- Dashboard Development

---

## 🔄 Analytics Process

### 1. Data Collection

Registration responses were collected and tracked using a structured registration database.

### 2. Data Preparation

The registration data was reviewed and organized for analysis, with relevant fields used to understand participant characteristics, registration sources, locations, and registration activity over time.

### 3. Data Analysis

The data was analyzed across key dimensions including:

- Registration volume
- Participant category
- Registration source
- Country
- Daily registration activity
- Weekly performance

### 4. Dashboard Development

The cleaned data was transformed into a Power BI dashboard containing KPIs, charts, participant breakdowns, geographic analysis, and registration trends.

---

## 📈 Dashboard

The final Power BI dashboard provides an overview of registration performance and campaign progress.

### Key Performance Indicators

| KPI | Result |
|---|---:|
| Total Registrations | **68** |
| Registration Target | **80** |
| Target Achievement | **85%** |
| Week 4 Registrations | **22** |
| Week 4 Growth | **69%** |

---

## 🔍 Key Insights

### Registration Performance

The campaign recorded **68 total registrations**, achieving **85% of the 80-registration target**.

### Week 4 Growth

Week 4 recorded **22 registrations**, representing **69% growth** and making it an important period of campaign momentum.

### Registration Sources

**WhatsApp** was the strongest registration source, generating **37 registrations**, followed by **word of mouth with 10 registrations**.

### Participant Categories

**Agribusiness owners** represented the largest participant category with **17 registrations**, followed by commercial farmers with **13** and smallholder farmers with **11**.

### Geographic Reach

Nigeria accounted for the majority of registrations, while the campaign also attracted participants from other countries across Africa.

---

## 💡 Recommendations

Based on the analysis, the following actions are recommended:

1. **Prioritize WhatsApp outreach** as a major registration acquisition channel.
2. **Continue targeting agribusiness owners and commercial farmers**, who represented strong participant segments.
3. **Analyze the drivers of Week 4 growth** and replicate successful campaign activities.
4. **Strengthen outreach in lower-performing geographic markets** to improve regional participation.
5. **Continue monitoring registration performance regularly** to identify gaps early and support timely campaign decisions.

---

## 🏆 Results

The analysis provided a clear view of registration performance and helped transform registration data into actionable campaign insights.

### Final Performance

**68 registrations achieved**

**85% of the 80-registration target**

**22 registrations in Week 4**

**69% Week 4 growth**

The dashboard provided a centralized view of campaign performance, participant composition, acquisition channels, and geographic reach.

---

## 📷 Project Screenshots

### Final Power BI Dashboard

![Final Power BI Dashboard](screenshots/final-dashboard.png)

### Registration Tracking

*Registration tracking screenshot will be added here.*

### Partnership / Stakeholder Responses

*Supporting data screenshot will be added here.*

---

## 📁 Project Structure

```text
cereal-africa-pan-african-summit-analysis/
│
├── README.md
│
├── data/
│   └── README.md
│
├── dashboard/
│   └── README.md
│
├── documentation/
│   └── README.md
│
└── screenshots/
    └── README.md
